function displayKey(key) {
  return key.charAt(0).toUpperCase() + key.slice(1).replace(/[A-Z]/g, c => ` ${c}`);
}
...
function validateLink(key, field) {
  ...
  if (!field.match(/^(https:\/\/)?([w|W]{3}\.)+[a-zA-Z0-9\-.]{3,}\.[a-zA-Z]{2,}(\.[a-zA-Z]{2,})?$/)) {
    return `${displayKey(key)} is not a valid url`;
  }
}

export const hasErrors = errors => {
  return Object.values(errors).some(errorGroup => Object.values(errorGroup).some(Boolean));
};
...
export default function validateForm(
  ...
  // Check required
  ['phoneNumber', 'gender', 'ethnicity', 'timezone'].concat(about_you.wantSwag ? [
    'addressLine1',
    'city',
    'province',
    'postalCode',
  ] : []).forEach(
    requiredValidator(about_you, errors.about_you),
  );
  ...
  if (!errors.about_you.phoneNumber && about_you.phoneNumber.match(/\d/g).length !== 10) {
    errors.about_you.phoneNumber = 'Invalid phone number';
  }

  if (about_you.wantSwag) {
    if (!errors.about_you.postalCode && !about_you.postalCode.match(
  ...
  if (!errors.your_experience.githubLink) {
    errors.your_experience.githubLink = validateLink('Github', your_experience.githubLink);
  }

  if (!errors.your_experience.portfolioLink) {
    errors.your_experience.portfolioLink = validateLink('Portfolio', your_experience.portfolioLink);
  }

  if (!errors.your_experience.linkedinLink) {
    errors.your_experience.linkedinLink = validateLink('Linkedin', your_experience.linkedinLink);
  }
